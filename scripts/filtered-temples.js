...
newt.addEventListener("click", () => {
    renderTemples(temples.filter(temple => {
        ...
        return newtemples > 1900;
    })
    )
});
large.addEventListener("click", () => {
    renderTemples(temples.filter(temple => {
        const largetemples = parseInt(temple.area)

        return largetemples > 90000;
    })
    )
});
...
small.addEventListener("click", () => {
    renderTemples(temples.filter(temple => {
        const smalltemples = parseInt(temple.area)

        return smalltemples < 10000;
    })
    )
});
...
function renderTemples(temple){
    const t = document.querySelector(".temples");
    let section = ""
    for (const info of temple) {
        section += `
        <section>
        <h1>${info.templeName}</h1>
        <p>Location: ${info.location}</p>
        <p>Dedicated: ${info.dedicated}</p>
        <p>Size: ${info.area}</p>
        <figure>
        <img src=${info.imageUrl} alt=${info.templeName} loading="lazy" width:"100">
        </figure>
        </section>
        `
    }
